...
import type { CreateContextOptions } from '@/components/fs/context-menu';
...
export default defineComponent({
  ...
  setup(props, { attrs, slots, emit, expose }) {
    ...
    function checkAll(all: boolean) {
      state.checkedKeys = all ? getEnabledKeys() : ([] as KeyType[]);
    }

    function expandAll(all: boolean) {
      state.expandedKeys = all ? getAllKeys() : ([] as KeyType[]);
    }
    ...
    function handleSearch(searchValue?: string) {
      if (searchValue !== searchState.searchText) searchState.searchText = searchValue as string;
    ...
    onMounted(() => {
      const level = Number(props.defaultExpandLevel);
      if (level > 0) {
