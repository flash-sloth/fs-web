...
export function useTree(treeDataRef: Ref<TreeDataItem[]>, getFieldNames: ComputedRef<FieldNames>) {
  ...
  function getChildrenKeys(nodeKey: Key, list?: TreeDataItem[]) {
    const keys: Key[] = [];
    const treeData = list || unref(treeDataRef);
    const { key: keyField, children: childrenField } = unref(getFieldNames);
    if (!childrenField || !keyField) return keys;
  ...
  // Update node
  function updateNodeByKey(key: Key, node: TreeDataItem, list?: TreeDataItem[]) {
    if (!key) return;
    const treeData = list || unref(treeDataRef);
    const { key: keyField, children: childrenField } = unref(getFieldNames);
  ...
  function filterByLevel(level: number, list?: TreeDataItem[], currentLevel = 1) {
    if (!level) {
      return [];
    }
    ...
    for (let index = 0; index < data.length; index++) {
      ...
      let varCurrentLevel = currentLevel;
      if (children && children.length && varCurrentLevel < level) {
        varCurrentLevel += 1;
        res.push(...filterByLevel(level, children, varCurrentLevel));
      }
    }
  ...
  // Delete node
  function deleteNodeByKey(key: Key, list?: TreeDataItem[]) {
    if (!key) return;
    const treeData = list || unref(treeDataRef);
    const { key: keyField, children: childrenField } = unref(getFieldNames);
    if (!childrenField || !keyField) return;
  ...
  // Get selected node
  function getSelectedNode(key: Key, list?: TreeItem[], defSelectedNode?: TreeItem | null) {
    if (!key && key !== 0) return null;
    const treeData = list || unref(treeDataRef);
    let selectedNode = defSelectedNode;
    treeData.forEach(item => {
      if (selectedNode?.key || selectedNode?.key === 0) return;
      if (item.key === key) {
        selectedNode = item;
        return;
      }
