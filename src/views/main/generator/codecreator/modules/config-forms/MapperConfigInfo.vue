...
function initFormData() {
  ...
  if (configData.mapperDesign) {
    formData.value = { ...configData.mapperDesign };
  }
}
  ...
  if (configData.mapperDesign) {
    for (const key in formData.value) {
      if (isValidKey(key, formData.value)) {
        let oldValue: any = configData.mapperDesign[key];
...
defineExpose<ConfigFormInstance>({
  ...
  validate: async () => {
    ...
    if (!err) {
      return { mapperDesign: { ...formData.value } };
    }
    return null;
  }
});
...
<template>
  <VxeForm ref="formRef" :rules="rules" :data="formData" :loading="loading" custom-layout>
    ...
    <VxeFormItem field="packageName" title="mapper包" title-colon :span="6">
      <template #default>
        <VxeInput v-model="formData.packageName" placeholder="基础包.模块包.mapper包" />
      </template>
    </VxeFormItem>
